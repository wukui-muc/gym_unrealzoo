...
class Track(UnrealCv_base):
    def __init__(self,
        ...
        if 'track_train' in self.env_name:
            self.objects_list=["cube1", "cube2_7", "cube3", "cube4", "cube5",
            ...
            for i in range(len(self.textures_list)):
                if self.docker:
                    self.textures_list[i] = os.path.join('/unreal', texture_dir, self.textures_list[i])
                else:
